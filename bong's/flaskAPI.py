...
def show_cart_item_in_cart(cart_item):
    db = sqlite3.connect(DB)
    cursor = db.cursor()
    cursor.execute('SELECT cart_item.ci_id, product_name, product_price,ci_quantity,product_photo FROM product INNER JOIN cart_item ON product.product_id=cart_item.ci_product_id WHERE cart_item.ci_cart_id=?', (str(cart_item),))
    rows = cursor.fetchall()
    print(rows)
...
def show_index_cart_item_in_cart(cart_item, cart_item_index):
    db = sqlite3.connect(DB)
    cursor = db.cursor()
    cursor.execute('SELECT ci_id, product_name, product_price,ci_quantity, product_photo FROM product INNER JOIN cart_item ON product.product_id=cart_item.ci_product_id WHERE cart_item.ci_cart_id=? AND cart_item.ci_id=?', (str(cart_item), str(cart_item_index)))
    rows = cursor.fetchall()
    print(rows)
